...
class SECDocumentProcessor:
    ...
    def chunk_document(self, doc_text: str, metadata: Dict) -> List[Dict]:
        ...
        for idx, chunk in enumerate(chunks):
            chunk_metadata = {
                "doc_id": metadata.get("accession_number") or metadata.get("doc_id"),
                "chunk_index": idx,
    ...
    def save_chunks(self, chunks: List[Dict], output_path: str = None):
        """
        Saves chunk data to a JSON file in the processed_text_chunk_path directory.
        Creates directory if it doesn't exist.
        """
        if output_path:
            output_file = Path(output_path)
        else:
            # Use the processed_text_chunk_path from settings
            output_dir = Path(self.settings.data.processed_text_chunk_path)
